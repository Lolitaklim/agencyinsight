...
function renderOrdinaryBlock(block) {
    const points = block.points.map((point)=>`<p class="point">${point.text}</p>`).join("");
    const html = `
...
function renderMarkNumList(block) {
    const points = block.points.map((point, index)=>{
        const ul = point.ul ? point.ul.map((item)=>`<li>${item}</li>`).join("") : "";
        return `
        <div class="point">
...
function renderContent(data) {
    ...
    document.querySelector('meta[name="description"]').setAttribute("content", caseData.description);
    document.querySelector('meta[name="keywords"]').setAttribute("content", caseData.keywords);
    console.log(caseData);
    const caseHead = caseHeadTmp(caseData.image_url, caseData.alt_image, caseData.title, caseData.category, caseData.logo_url, caseData.alt_logo);
    ...
    const alsoCases = data.also_cases.map((item)=>alsoCaseTmp(`#${item.href}`, item.image_url, item.alt_image, item.title, item.category)).join("");
...
async function loadCase() {
    try {
        ...
        const response = await fetch(`${SERVER_URL}/api/get_case.php?case=${encodeURIComponent(caseId)}`);
...
//# sourceMappingURL=case.b7df8914.js.map
